...
`pagepool` 改大对 RDMA 场景收益明显，但它是**独占的物理内存**，容量规划时必须算进去，别调完直接 OOM。
...
**注意最后一行**：大块顺序传输时两者差别很小，因为那是带宽受限，协议栈开销被摊薄了。RDMA 的价值集中在**小 I/O 的延迟和 CPU 占用**上 —— 这也是判断「值不值得上」的依据。
...
GDS 的开关在应用侧（要用 cuFile 写代码），存储运维负责的是**把条件准备好并证明它可用**：文件系统选型支持 GDS、RDMA 链路正常、GPU 与网卡的 PCIe 拓扑合理、IOMMU 状态正确。
...
<Quiz
  ...
  explain={<>
    GPFS 在 RDMA 建连失败时会静默回落 TCP —— 保可用是对的，但意味着「没提升也没报错」正是回落的典型症状。
...
<Quiz
  ...
  explain={<>存储运维在这件事上的职责是把条件准备好并给出证据 —— 跑 <code>gdscheck -p</code>，而不是查产品手册。</>}
/>
