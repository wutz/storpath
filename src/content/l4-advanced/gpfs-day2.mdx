...
GPFS 的 Day-2 和 Ceph 的 Day-2 关注点不同。Ceph 那边主要是盘和 PG，GPFS 这边更多是多租户、配额、导出和扩容，因为它通常部署在要给多个部门/项目分配空间的企业环境里。
...
- 独立集群：性能完全物理隔离，功能最完整。但起步容量大（用不完就是空置）、建设周期长，集群一多，交付人力也撑不住
- 独立文件系统：能按 50T 步长满足中等需求，交付也快。但 50T 步长还是不够细；GUI 的 CSI 权限不能按文件系统维度控制，所以用不了原生 GPFS CSI，只能用功能更弱的第三方 CSI
...
- 三种多租户方案的取舍：物理集群隔离最好、成本最高，文件系统次之，fileset 最快但竞争最严重
