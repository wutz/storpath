...
到这里，L0 的知识点已经齐了。这些工具可以组织成几个可复用的动作：遇到什么问题，按什么顺序，敲哪些命令。
...
顺序永远是先用零开销的工具收敛范围，再用追踪工具挖根因。反过来做，你会在生产机器上跑一个高开销的 trace，然后被业务方投诉。

## elbencho 的压测四件套

压测不是「跑个分」，是回答一个具体问题。参数跟着问题走。
...
单机压出来的数字只是上限参考。共享文件系统（CephFS、GPFS）要靠多客户端并发才能压出真实能力，同一个 elbencho 加上 service 模式就能搞定。
...
压测裸盘时同步开一个 `iostat -xm 1` 观察。按经验值，NVMe 单盘写入约 5 GB/s，如果实测差得远，先怀疑 PCIe 链路降速：
...
elbencho 的结果表有两列数字，很多人只看右边一列就走了。这两列之间的差距本身就是结论。
...
- **last done** = 最后一个线程干完时的统计，等于整个作业的真实完成时间。这一列包含了后半段「快的已经收工、慢的还在磨」的拖尾，它才是你真正能对外承诺的聚合数字。

上面这组数据里 last done 的带宽只有 first done 的六成，最快和最慢的线程差了 27 秒。这不是一次可信的压测，是一次暴露了不均衡的压测。

想知道「这堆盘、这张网到底能跑多快」，看 first done；想知道「这批活干完要多久」，看 last done。

<Callout type="tip" title="两列各有各的用途，不要只留一个">
- 判断硬件上限用 first done，它更接近设备/网络的理论峰值
- 描述业务感知（「全部完成要多久」）用 last done
- 两列的差距用来判断这次测试有没有资格当基线。经验阈值是 elapsed 差 10% 以内正常，超过 20% 先查不均衡，别急着记数字
</Callout>

<Callout type="tip" title="判断硬件上限、做验收对比理论值，一定用 first done">
硬件验收要回答的是「这批盘和这张网有没有跑到它该有的水平」，这时只有 first done 有意义。它是所有线程都还在跑、机器满载时的数字，最接近设备与链路的物理峰值。last done 已经被拖尾稀释过，拿它对比理论值会系统性地低估硬件。

12 块 NVMe 单盘写 5 GB/s，理论上限约 60 GB/s；节点是 2 × 25GbE，网络上限约 6 GB/s。

- first done 跑到 4.9 GB/s，达到网络理论值的 80% 以上 → 硬件没问题，瓶颈在网络，符合预期，可以验收
...
last done 只是这一次运行的完成时间，它把所有拖尾原封不动地算进去了。如果差距来自数据量太小、某块慢盘或后台 recovery，那这个数字描述的是那次故障或那个测试缺陷，不是系统能力。直接抄进报告，等于把偶然当结论，而且换台机器就复现不出来。

出数字之前按顺序做四件事：

1. **先看差距**。超过 20% 不出数字，原因解决或解释清楚再重跑一轮
...
`--timelimit` 是按时间跑，所有线程同时被叫停，first done 和 last done 自然几乎相等。差距消失不等于不均衡消失，它只是换了个形式，表现为总吞吐比预期低。所以定位不均衡时要跑固定数据量（只给 `-s` / `--randamount`，不加 `--timelimit`）让快慢差异显形。打稳态基线才用 `--timelimit`。
...
静态工具告诉你「盘的平均延迟是 15ms」，但你需要知道是谁在发这些慢 I/O，这时就该上 bpftrace 了。
...
<Quiz
  ...
  options={[
    { text: '这块盘性能超出标称，是好事', feedback: '大幅超出标称值，多半是测试方法有问题，不是中彩票。' },
...
<Quiz
  ...
  options={[
    { text: '相信平均值，判断不是存储问题', feedback: '平均值最容易掩盖长尾。' },
    { text: '用 biolatency 看延迟分布直方图，确认是否存在长尾', correct: true },
    { text: '直接换盘', feedback: '没有任何证据支持换盘。' },
    { text: '扩容集群', feedback: '偶发卡顿与容量无关。' },
  ]}
  explain={<>偶发卡顿基本都是长尾问题，平均值看不到，得看分布。biolatency 的直方图能直接暴露那几十个 16ms 以上的请求，再用 biosnoop 定位到具体进程。</>}
/>
...
<Quiz
  ...
  explain={<>
    两列差了 40%，最快和最慢的线程完成时间差了近 27 秒。常见嫌疑依次是某块盘慢（PCIe 降速、降频、坏道重试）、任务切分不均、数据分布不均、后台 scrub/recovery 在抢资源。
    先用 iostat 或 ceph osd perf 找出那个拖尾的对象，修掉再重跑。差距收进 10% 以内的那一轮，才有资格当基线。
  </>}
/>

## 把 elbencho 的两列数字和 bpftrace 的长尾一起交上去

- 范围收敛交给零开销的静态工具，挖根因再上 bpftrace 类工具
- elbencho 四件套的用法是顺序读写测带宽、随机读写测 IOPS；`--direct` 必加，`--timelimit` 得给够时长
- 同一个 elbencho 加 `--service` / `--hosts` 就能多客户端压共享文件系统，多文件元数据场景更要压
- 结果表别只看一列：first done 是满载峰值，last done 是这次运行的聚合完成情况。差距超过 20% 先查不均衡（数据量太小、慢盘、切分不均、分布不均、后台任务），别急着记数字
...
- 报告必须写清方法、环境，以及没测过哪些场景

L0 到此结束。你现在有能力在任何一台 Linux 机器上定位性能问题，接下来把这套能力用到分布式存储上。
