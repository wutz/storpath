...
<Quiz
  id="day2-1"
  question="你要重启一台存储节点做内核升级，该怎么走？"
  options={[
    { text: '直接重启，Ceph 会自己处理', feedback: '默认 10 分钟后这些 OSD 会被标记 out，白白触发一轮全量迁移。' },
    { text: 'ceph osd set noout → 重启 → 确认 OSD 全部 up → ceph osd unset noout', correct: true },
    { text: '先把该节点所有 OSD purge 掉，重启后重新加回', feedback: '要触发两次全量数据迁移，代价极大。' },
    { text: '把 min_size 改成 1 再重启', feedback: '完全没必要，还平白引入丢数据的风险。' },
  ]}
  explain={<>
    noout 告诉集群「这些 OSD 只是暂时不在，别急着重新分布数据」。
    重启是计划内的短暂离线，用不着迁几十 TB 数据。用完记得 unset。
  </>}
/>

<Quiz
  id="day2-2"
  question="集群里已经有 OSD 到 88% 使用率，这时候一块盘坏了，下一步怎么走？"
  options={[
    { text: '立刻 ceph osd out，越快恢复冗余越好', feedback: '迁移会推高其它 OSD，很可能把 88% 那块顶到 full 导致整池拒写。' },
    { text: '先评估容量余量和迁移影响，必要时限速，再执行 out', correct: true },
    { text: '把 full_ratio 调到 0.99 争取空间', feedback: 'full_ratio 是最后的应急手段，不是第一步，而且极度危险。' },
    { text: '什么都不做，等它自己恢复', feedback: '降级状态长期挂着，同样是风险。' },
  ]}
  explain={<>
    换盘的风险不在换盘本身，而在它触发的迁移，上一关闯关里强调过。
...
<Quiz
  ...
  options={[
    ...
    { text: '直接 unset，反正是别人设的', feedback: '对方要是还在维护中途，贸然取消会触发意外迁移。' },
    { text: '把它写进告警屏蔽规则', feedback: '屏蔽掉之后，下次真出事就没人知道了。' },
  ]}
  explain={<>忘记取消 noout 是常见事故，真坏盘时集群不会自动恢复，数据长期降级也无人察觉。变更记录和交接文档就是为这种时候准备的。</>}
/>
...
- 换盘要先把容量算清，别让迁移把集群顶到 full
- 计划内重启用 `noout`，用完马上取消
- 集群 HEALTH_OK 了才能升级，期间别夹带别的变更
