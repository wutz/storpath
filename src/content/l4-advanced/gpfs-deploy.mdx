...
这是 GPFS 部署里翻车最多的一处，没有例外。部署前用 `pdsh -w ^all md5sum /etc/hosts` 确认所有节点的 hosts 文件哈希一致。
</Callout>
...
按 L1 和 L3 学的方法算，另外 ECE 还有两项自己的开销要算进去：
...
declustered array（DA）是 ECE 里的关键机制。同一 RG 内所有节点的同类磁盘组成一个分散阵列，数据和校验摊到 DA 的所有磁盘上。重建时所有盘一起参与，速度远快于传统 RAID 那种"往一块热备盘写"。
...
选错了只能重建文件系统。所以需求调研阶段就得问清楚一件事：平均文件大小是多少。
</Callout>

## 第 6 步：调优（先动这四个参数）

下面这几个参数是 GPFS 调优绕不开的，值来自实际项目：
...
内存这笔账得算清。`3M × 8K ≈ 24GB` 只是 inode 缓存，再加上 `4M × 500B ≈ 2GB` 的 stat 缓存，还有 pagepool。这些内存要在容量规划阶段就留出来，不然调完参数节点直接 OOM。
</Callout>
...
- DA 让所有盘一起参与重建，这是 ECE 重建快的原因；扩容要同型号、同数量
- `block-size` 创建后改不了，需求调研时就得问清平均文件大小
- 调优先动这四个：`maxMBpS`、`workerThreads`、`maxFilesToCache`、`maxStatCache`
