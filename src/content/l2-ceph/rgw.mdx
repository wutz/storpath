...
RGW（RADOS Gateway）是一个 HTTP 服务，把 S3 / Swift API 翻译成 RADOS 操作。和 RBD、CephFS 不一样，客户端不用装任何 Ceph 客户端，一个 HTTP 请求就够了。
...
`buckets.index` 存的是每个 bucket 里所有对象名的有序索引。它的特点是小对象、高频随机读写、omap 密集。

放在 HDD 上，`ls` 一个大 bucket 会慢到超时；单个 bucket 对象数过多（百万级以上）时，索引分片不足还会导致写入阻塞。自建对象存储的性能事故，大半出自这里。

index 池用副本 + SSD，data 池才用 EC，这条别记反。
</Callout>

## 把 RGW 跑起来

cephadm 一条命令就能拉起 RGW。
...
<Callout type="trap" title="接入 RGW 最常撞上的三个报错">
| 报错 | 原因 | 处理 |
| --- | --- | --- |
...
规划口径是每个索引分片约 10 万个对象。预计存 1000 万对象的 bucket，分片数要在 100 以上。

<Callout type="warn" title="动态 reshard 要付出的代价">
新版本 RGW 支持自动 reshard，但重分片期间该 bucket 的写入会被阻塞（大 bucket 可能持续几分钟）。多站点复制场景下自动 reshard 还可能引发同步问题。

最好的办法是建 bucket 时就把分片数设够，别等它长大了再补救，那会儿补救要停写。
</Callout>
...
第一条规则（7 天清理未完成的分段上传）该是每个 bucket 的标配。上一阶段说过，残留分片是「用量对不上」的头号原因。
...
<Quiz
  id="rgw-1"
  question="自建 RGW 存海量小对象，跑了几个月之后写入变慢、ls bucket 经常超时。你先查什么？"
  options={[
  ...
  explain={<>
    ...
    这两点没做对，海量对象场景一定会出问题，自建对象存储最经典的坑就在这里。
  </>}
/>

<Quiz
  id="rgw-2"
  question="关于 RGW 的池规划，下面哪些说法站得住？（多选）"
  options={[
  ...
  explain={<>规划原则是数据大而顺序 → EC，元数据小而随机 → 副本 + SSD。这个原则在 CephFS 上同样成立。</>}
/>
...
<Quiz
  ...
  explain={<>
    2000 万 ÷ 10 万 = 200 片。建 bucket 时就设够，别让后面的 reshard 阻塞写入。
    这个数字，设计阶段就该问清楚：这个 bucket 预计存多少对象？
  </>}
/>

## RGW 上线前要盯住的几件事

- RGW 是无状态 HTTP 网关，客户端不用装 Ceph 客户端
- index 池必须副本 + SSD，data 池才用 EC
- 高频报错就那三个：签名不匹配查时间、域名风格、时间偏移
- bucket 分片按每片 10 万对象规划，建的时候就设够，别事后 reshard
- 每个 bucket 都配上「7 天清理未完成分段上传」的生命周期规则
