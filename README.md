...
一条存储运维工程师的成长路线，全程在线交互，没有分岔。弄懂存储在说什么，
学会读机器给的信号，亲手把集群跑起来、扛住它出的故障，
最后替业务算账，走进 RDMA 网络、GPFS ECE、K8s CSI 与商业存储的进阶战场。

## 课程阶段

课程按难度排成一条线，从零基础开始一节一节往下走即可。顺序只是建议，
有底子的可以直接跳到对应阶段。
...
排序有两条原则。**先把语言立起来**：连「块 / 文件 / 对象」都分不清，去读 iostat
就找不到落点。**先跑起来**：L2 把 cephadm 部署提到 CRUSH/PG 之前，
让 `ceph -s` 里的 PG 数字先出现在眼前，再回头解释它怎么算出来的。

共 5 个阶段 **38 节课，正文全部写完**，其中动手环节 19 个（13 个实验 + 4 个命令行闯关 + 2 个规划计算器），
...
- **规划计算器（Planner）**：容量推算（TB→TiB、冗余开销、水位预留三刀账）和
...
> MDX 里 JSX 属性值用双引号包裹，属性内部不要再出现半角双引号（用 `「」` 代替），
> 否则会在构建时报解析错误。
...
命令行闯关：给命令加 `goal` 字段就成了闯关目标，全部达成后自动记录通过。
