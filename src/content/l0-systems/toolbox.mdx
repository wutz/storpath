...
fio --name=seqwrite --filename=/dev/nvme0n1 --rw=write \
    --bs=1M --iodepth=32 --numjobs=4 --ioengine=libaio --direct=1 \
...
fio --name=randwrite ... --rw=randwrite --bs=4k --iodepth=64 ...
...
| 参数 | 含义 | 常见错误 |
| --- | --- | --- |
| `--direct=1` | 绕过 page cache | **忘了加，测出的是内存速度** |
| `--bs` | 块大小 | 不写块大小的 IOPS 数字没有意义 |
| `--iodepth` | 单线程队列深度 | 只测 iodepth=1 会低估设备能力 |
| `--numjobs` | 并发线程数 | 单线程压不满 NVMe |
| `--time_based --runtime` | 按时间跑而非按数据量 | 不加可能只测到 SSD 的缓存区间 |

<Callout type="trap" title="三个让压测结果失真的经典错误">
1. **忘了 `--direct=1`** —— HDD 跑出 3GB/s，测的是 page cache
2. **测试时长太短** —— SSD 有 SLC 缓存，前 30 秒很快，之后掉到稳态。至少跑 5 分钟才能看到真实的稳态性能
3. **在生产盘上跑写测试** —— `--filename=/dev/nvme0n1` 会直接覆盖裸设备上的数据。**动手前先确认这块盘是空的**
</Callout>

## 压测：elbencho 测共享文件系统

fio 测的是单机块设备。共享文件系统（CephFS、GPFS）需要**多客户端并发**才能压出真实能力，这时用 elbencho——它由 BeeGFS 创始人开发，一个工具覆盖了 fio、mdtest、ior 的功能，同时支持文件系统、S3 和块设备。
...
<Quiz
  id="tool-1"
  question="用 fio 测一块企业级 NVMe 的顺序写，跑了 30 秒得到 6.8 GB/s，远超厂商标称的 5 GB/s。最可能的原因是？"
  options={[
    { text: '这块盘性能超出标称，是好事', feedback: '大幅超出标称值通常意味着测试方法有问题，而不是中彩票。' },
    { text: '忘了加 --direct=1，测的是 page cache', correct: true },
    { text: 'PCIe 链路超频了', feedback: 'PCIe 不会自动超频。' },
    { text: '块大小设得太大', feedback: '块大小影响效率但不会让结果超过物理上限。' },
  ]}
  explain={<>
    没有 --direct=1 时数据先进 page cache，write() 立刻返回，测到的是内存带宽。
    另一个常见原因是测试时长太短，只覆盖了 SSD 的 SLC 缓存区间——所以要 --time_based 跑够 5 分钟看稳态。
  </>}
/>
...
- fio 四件套：顺序读写测带宽、随机读写测 IOPS；`--direct=1` 必加，时长要够
- elbencho 用于多客户端压共享文件系统，尤其是多文件元数据场景
