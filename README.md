...
| 路线 | `?role=` | 面向 | 规模 |
| --- | --- | --- | --- |
| **解决方案架构师** | `architect` | 出方案、算账、做选型对比，每个数字都得站得住 | 15 节 / 约 8 小时 |
| **计算集群运维工程师** | `cluster-ops` | 存储是依赖而非产品，要挂得上、跑得快、说清是谁的锅 | 17 节 / 约 10 小时 |
| **存储运维工程师** | `storage-ops` | 不做裁剪的完整主线，L0→L4 按阶段通读 | 36 节 / 约 23 小时 |

前两条按裁剪过的分段清单渲染（连续编号 + 来源阶段徽标），存储运维那条是完整主线，
