...
不过流程和参数还是得掌握。面试、方案评审、跟厂商沟通都会用到，也能解释它为什么比 CephFS 快。
</Callout>
...
GPFS 部署里翻车最多的就是这里，没有例外。部署前用 `pdsh -w ^all md5sum /etc/hosts` 确认所有节点的 hosts 文件哈希一致。
</Callout>
...
官方要求：高性能环境下 CES 节点除导出服务外不应运行其它工作负载；CES 协议访问用的网络也必须和 ECE 内部流量用不同的物理网卡、不同的网络。

理由跟 Ceph 的 public/cluster 分离一样。导出服务的客户端流量会和 ECE 的纠删码流量抢带宽，而纠删码流量本身就在数据路径上。
</Callout>

## 第 2 步：算容量和 EC 方案

按 L1 和 L3 学的方法算，ECE 另外还有两项自己的开销要算进去。
...
declustered array（DA）是 ECE 里的关键机制。同一 RG 内所有节点的同类磁盘组成一个分散阵列，数据和校验摊到 DA 的所有磁盘上。

重建时所有盘一起参与，速度远快于传统 RAID 那种「往一块热备盘写」。
...
ECE 用安装工具包（installation toolkit）部署，流程不算复杂。
...
GPFS 的块大小对性能影响很大，而且创建后改不了。

- 大文件顺序读写为主（AI 训练、HPC）→ 8M / 16M
- 小文件多、元数据密集 → 1M / 4M
- 混合负载 → 4M 起步

选错了只能重建文件系统，所以需求调研阶段就得问清楚平均文件大小是多少。
</Callout>

## 第 6 步：调优，先动这四个参数

这几个参数是 GPFS 调优绕不开的，值来自实际项目。
...
`maxFilesToCache` 和 `maxStatCache` 直接决定了 `ls`、`stat` 这类操作能否命中内存。海量小文件场景下把这两个参数调对，元数据性能能差出数量级。

内存这笔账得算清。`3M × 8K ≈ 24GB` 只是 inode 缓存，再加上 `4M × 500B ≈ 2GB` 的 stat 缓存，还有 pagepool。

这些内存要在容量规划阶段就留出来，不然调完参数节点直接 OOM。
</Callout>
...
<Quiz
  ...
  options={[
    ...
    { text: '数据盘容量不够', feedback: '集群创建阶段还不涉及数据盘。' },
    { text: 'EC 方案选错了', feedback: 'EC 在创建 vdisk 阶段才涉及。' },
    { text: '直接重装操作系统', feedback: '代价太大，也没针对到问题。' },
  ]}
  explain={<>
    GPFS 部署的头号翻车点就在这里。用 <code>pdsh -w ^all md5sum /etc/hosts</code> 确认所有节点哈希一致，
...
<Quiz
  ...
  options={[
    ...
    { text: '参数值设得太小', feedback: '完全没变化的话，先确认参数是否真的生效。' },
  ...
  explain={<>
    ...
    多数参数需要 <code>mmshutdown -a &amp;&amp; mmstartup -a</code> 才生效。排障时这两个命令的区别得分清。
  </>}
/>
...
## ECE 这套集群从零到跑起来，最容易在哪一步返工？

- 预检没做干净，后面就是反复返工；`/etc/hosts` 三段格式加全节点一致是头号坑
- 角色在写集群定义之前定下来，quorum 数看节点数，CES 独占，网络分开
- DA 让所有盘一起参与重建，所以 ECE 重建快；扩容要同型号、同数量
- `block-size` 创建后改不了，需求调研时就得问清平均文件大小
- 调优先动这四个参数，`maxMBpS`、`workerThreads`、`maxFilesToCache`、`maxStatCache`
- 元数据缓存参数调之前先算内存，算不清就是 OOM
- 改完参数 `mmshutdown -a && mmstartup -a`，再用 `mmdiag --config` 确认真的生效了
