...
课程只有一份（下面的 L0–L4），首页按岗位重新排了三条走法 —— 同一节课可以出现在多条路线里，
每条路线只挑这个岗位真正用得上的部分。路线用 `?role=` 记在 URL 上，可以直接分享。

| 路线 | 面向 | 规模 |
| --- | --- | --- |
| **解决方案架构师**（方案工程师、售前技术） | 出方案、报配置、做选型对比，每个数字都得站得住 | 15 节 / 约 8 小时 |
| **服务工程师 / 计算集群运维**（GPU 集群、K8s 平台） | 存储是依赖而非产品，要挂得上、跑得快、说清是谁的锅 | 17 节 / 约 10 小时 |
| **存储运维工程师**（存储 SRE） | 站点原来的那条完整主线，L0→L4 一节不落 | 36 节 / 约 23 小时 |

三条路线共用同一份进度（localStorage），在一条路线里完成的课在另一条里也是完成状态。
定义见 `src/lib/roles.ts`；不在任何岗位路线里的课（Ceph 深度运维、GPFS 部署等）仍在完整主线与
首页的「完整课程目录」里。
